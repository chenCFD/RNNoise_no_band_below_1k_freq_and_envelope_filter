...
def mycost(y_true, y_pred):
    return K.mean(mymask(y_true) * (10*K.square(K.square(K.sqrt(y_pred) - K.sqrt(y_true))) + K.square(K.sqrt(y_pred) - K.sqrt(y_true)) + 0.01*K.binary_crossentropy(y_pred, y_true)), axis=-1) + 0.1*K.mean(mymask(y_true) * (K.square(K.sqrt(y_pred))))
...
print('Build model...')
main_input = Input(shape=(None, 57), name='main_input')
...
denoise_output = Dense(37, activation='sigmoid', name='denoise_output', kernel_constraint=constraint, bias_constraint=constraint)(denoise_gru)
...
x_train = all_data[:nb_sequences*window_size, :57]
x_train = np.reshape(x_train, (nb_sequences, window_size, 57))

y_train = np.copy(all_data[:nb_sequences*window_size, 57:94])
y_train = np.reshape(y_train, (nb_sequences, window_size, 37))

noise_train = np.copy(all_data[:nb_sequences*window_size, 94:131])
noise_train = np.reshape(noise_train, (nb_sequences, window_size, 37))

vad_train = np.copy(all_data[:nb_sequences*window_size, 131:132])
...
checkpoint_cb = ModelCheckpoint(
        'weights.{epoch:03d}-{val_loss:.2f}.hdf5', save_weights_only=False, save_freq='epoch')
